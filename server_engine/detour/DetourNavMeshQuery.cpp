...
dtStatus dtNavMeshQuery::findInsidePolyPoint( const float* center, const float * extents, 
                                    const dtQueryFilter* filter, const  float maxdist, float *insidePt ) const
{
	dtAssert(m_nav);
	...
	// Find nearest polygon amongst the nearby polygons.
    const dtPoly* nearestPoly = 0;
    const dtMeshTile *nearestTile = 0;
	float nearestDistanceSqr = FLT_MAX;
	float closestPtPoly[3];
	...
	for (int i = 0; i < polyCount; ++i)
	{
		...
		if (d < nearestDistanceSqr)
		{
            ...
            nearestPoly = poly;
            nearestTile = tile;
            dtVcopy( nearestPos, closestPtPoly );
		}
	}
    ...
    {
        float x = 0, y = 0, z = 0;
        int count = nearestPoly->vertCount;
        for( int i =0; i < count; i++ )
        {
            int index = nearestPoly->verts[i] * 3; 
            x += nearestTile->verts[index];
            y += nearestTile->verts[index + 1];
            z += nearestTile->verts[index + 2];
        }
        insidePt[0] = x / count;
        insidePt[1] = y / count;
        insidePt[2] = z / count;
        float diff_x = insidePt[0] - nearestPos[0];
        float diff_z = insidePt[2] - nearestPos[2];
        float sqr_diff = diff_x * diff_x + diff_z * diff_z;
        if( sqr_diff > maxdist * maxdist ) 
        {
            insidePt[0] = nearestPos[0] + diff_x * maxdist;
            insidePt[2] = nearestPos[2] + diff_z * maxdist;
        }
        return DT_SUCCESS;
    }
    else
								 ...
								 float* t, float* hitNormal, dtPolyRef* path, int* pathCount, const int maxPath) const
{
	...
	*t = 0;
	...
	while (curRef)
	{
		...
		if(!filter->passFilter(0, 0, poly))//12
		{
			if (pathCount)
				*pathCount = n;
			return status;
