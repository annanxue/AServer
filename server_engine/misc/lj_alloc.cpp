...
#define DEFAULT_GRANULARITY	((size_t)2U * (size_t)1024U * (size_t)1024U)
#define DEFAULT_TRIM_THRESHOLD	((size_t)2U * (size_t)1024U * (size_t)1024U)
#define DEFAULT_MMAP_THRESHOLD	((size_t)2U * (size_t)1024U * (size_t)1024U)
...
void lj_mem_info( void* msp, const char* sys )
{
    FILE* f_mem = fopen( "lj_mem_info.log", "a+" );
    if( f_mem ) {
        time_t tm_now;
        char time_str[256] = {0};
        if( time( &tm_now ) == -1 ) {
            memcpy( time_str,"Unknown Time!!", 15 );
        } else {
            struct tm tm_time;
            localtime_r( &tm_now, &tm_time );  
            strftime( time_str, 256, "[%Y-%m-%d %H:%M:%S] ", &tm_time );
        }
        fprintf( f_mem, "-----%s mem info at time: %s-----\n", sys, time_str );
        mstate m = (mstate)msp;
        msegmentptr sp = &m->seg;
        int ct = 1;
        int total = 0;
        while(sp != 0) {
            if( (char*)m->top >= sp->base && (char*)m->top < sp->base + sp->size ) {
                fprintf( f_mem, "   chunk %d [top]  --> base: %p, size: %d\n", ct++, sp->base, sp->size );
            } else {
                fprintf( f_mem, "   chunk %d --> base: %p, size: %d\n", ct++, sp->base, sp->size );
            }
            total += sp->size;
            sp = sp->next;
        }
        fprintf( f_mem, "-----%s chunk count: %d, dv size: %d, top size: %d, total mem size: %d byte-----\n\n", sys, --ct, m->dvsize, m->topsize, total );
        fclose( f_mem );
    }
}
