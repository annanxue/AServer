module( "db_migrate", package.seeall )
...
g_db_gmserver = g_db_gmserver or DataBase:c_new()
g_db_gmserver_init = g_db_gmserver_init or nil 
...
UPDATE_PLAYER_INSERT_SQL_FORMAT = "issiiiffffiiiiiiiiiiiiiissssiiisiiiiiiiiiiiiiiiiisiii"
...
function update_account_logon(_account_id, _client_ip_str)
    if dbserver.g_db_character:c_modify( sformat( UPDATE_ACCOUNT_LOGON_SQL, _client_ip_str, os.time(), _account_id ) ) < 0 then
        c_err( "[db_migrate](update_account_logon) _account_id: %d; update sql error", _account_id )
    end
end
...
function set_account_register_ip(_account_id, _client_ip_str)
    if dbserver.g_db_character:c_modify( sformat( SET_ACCOUNT_REGISTER_IP_SQL, _client_ip_str, _account_id ) ) < 0 then
        c_err( "[db_migrate](set_account_first_player_time) _account_id: %d; update sql error", _account_id )
    end
end
...
local function do_create_account( _account_id, _client_ip_str )
    ...
    if database:c_modify( sformat( DEFAULT_ACCOUNT_INSERT_SQL, _account_id, _client_ip_str, _client_ip_str, os.time()  ) ) < 0 then
      --  c_err( "[db_migrate](do_create_accont)sql exec error!" )
    end
...
INC_ACCOUNT_TBL_INSERT_SQL = [[
    INSERT INTO INC_ACCOUNT_TBL 
    (
        `account_name`
    )
    VALUES
...
local function save_player_to_db( _player  )
    ...
    local time = c_cpu_ms()
    local user_name = "user" .. os.time()
    if g_db_gmserver:c_modify( sformat(INC_ACCOUNT_TBL_INSERT_SQL , user_name ) ) < 0 then 
        c_err("save_player_to_db => INC_ACCOUNT_TBL_INSERT_SQL error ")
        return 
    end 

    local insert_account_id  = g_db_gmserver:c_get_insert_id()
    ...
    _player.account_id_  =  insert_account_id 
    _player.account_name_ = "" .. insert_account_id
    _player.player_name_ = "name_" .. os.time() 
    ...
    -- 先插入一条默认的
        -- do insert player
            if database:c_modify( sformat(
                DEFAULT_PLAYER_INSERT_SQL,
                _player.account_id_,
                _player.player_name_,
                _player.account_name_,
                _player.model_id_,
    ...
    -- 关联账号表
    do_create_account(_player.account_id_ , "127.0.0.1")
    local player_list = {}
    player_list[ _player.player_id_ ] =
    {
        player_name_ = _player.player_name_,
        model_id_ = _player.model_id_,
        use_model_ = 1,
        model_level_ = 1,
        job_id_ = _player.job_id_,
        slot_ =  1,
        level_ = _player.level_,
        total_gs_ = _player.total_gs_,
        is_db_new_ = true,
        is_account_new_ = true,
        is_player_new_ = true,
        need_download = 0,

    } 
    if not do_update_account( _player.account_id_, player_list, _player.player_id_ ) then
        return 
    end
    ...
    if database:c_stmt_format_modify( UPDATE_PLAYER_INSERT_SQL,UPDATE_PLAYER_INSERT_SQL_FORMAT , 
        ...
        c_err( "[db_login](db_migrate -> save_player_to_db)update db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return 
    end

    if db_bag.do_save_player_bag( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update bag db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_bag.do_save_player_warehouse( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update warehouse db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_skill.do_save_player_skill_talent( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update skill db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_element.do_save_player_element( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update element db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_rune.do_save_player_rune( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update rune db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end
    
    if db_quest.do_save_player_quest( _player ) < 0 then 
         c_err( "[db_login](db_migrate -> save_player_to_db)update quest db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end
   
    if db_avatar.do_save_player_avatar( _player ) < 0 then
        c_err( "[db_login](sdb_migrate -> ave_player_to_db)update avatar db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_soul.do_save_player_soul( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_do_db)update soul db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_mastery.do_save_player_mastery( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_do_db)update mastery db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_mail.do_save_player_mail( _player ) < 0 then
        c_err("[db_login](db_migrate -> save_player_to_db)update mail db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end
    
    if db_pray.do_save_player_pray( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update pray db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end
   
    if db_time.do_save_player_time( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update time db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_newbie_guide.do_save_player_newbie_guide_record( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update newbie guide record db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_mystic.do_save_player_mystic( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update mystic db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if do_save_player_all_system_data( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update system data error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end 
  
    if db_tower.do_save_player_tower( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db) update tower db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end
    
    if db_palace.do_save_player_palace( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db) update palace db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end    

    if db_daily_goals.do_save_player_daily_goals_record( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update daily goals record db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_attend.do_save_player_attend( _player ) < 0 then
        c_err( "[db_attend](db_migrate -> save_player_to_db) update attend db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_rmb.do_save_player_rmb( _player ) < 0 then
       c_err( "[db_login](db_migrate -> save_player_to_db)update rmb db error! player_id: %d", _player.player_id_ )
       database:c_rollback()
       return
    end

    if db_snapshot.do_save_player_snapshot( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update snapshot db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_guild.do_save_player_guild( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update guild db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end
    
    if db_friend.do_save_player_friend( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update friend db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_inst.do_save_player_inst( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update inst db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end
    
    if db_truncheon.do_save_player_truncheon( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update truncheon db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_guild_profession.do_save_player_guild_profession( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update guild_profession db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_equip_exchange_shop.do_save_player_equip_exchange_shop_record( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update equip exchange shop record db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_title.do_save_player_title( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update title db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_permanent_attr_record.do_save_player_permanent_attr_record( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update permanent attr record db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_welfare.do_save_player_welfare( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update welfare db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_trade.do_save_player_trade( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update trade db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_vip.do_save_player_vip( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update vip db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_auction.do_save_player_auction( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update auction db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_guild_donation.do_save_player_guild_donation( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update guild_donation db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_guild_pve.do_save_player_guild_pve( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update guild_pve db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_prevent_kill_cheat.do_save_player_prevent_kill_cheat( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update prevent_kill_cheat db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_customer_service.do_save_player_customer_service( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update customer_service db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_god_divination.do_save_player_god_divination( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update god_divination db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_red.do_save_player_red( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update red db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_share.do_save_player_share( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update share db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_level_reward.do_save_player_level_reward( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update level_reward db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_pet.do_save_player_pet( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update pet db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_celebration.do_save_player_celebration( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update celebration db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_holiday_signin_award.do_save_player_holiday_signin_award( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update holiday_signin_award db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_chat.do_save_player_chat( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update chat db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_monthly_signin_award.do_save_player_monthly_signin_award( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update monthly_signin_award db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_mall.do_save_player_mall( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update mall db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_daily_online_award.do_save_player_daily_online_award( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update daily_online_award db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_cycled_recharge_award.do_save_player_cycled_recharge_award( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update cycled_recharge_award db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_growth_fund.do_save_player_growth_fund( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update growth_fund db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_login_activity.do_save_player_login_activity( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update login_activity db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_player_team_ladder.save_player_team_ladder_data_all( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update player team ladder data db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_sanding.do_save_player_sanding( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update sanding db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_survey.do_save_player_survey( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update survey db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_small_data.do_save_player_small_data( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update small_data db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_battle_justice.do_save_player_battle_justice( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update battle_justice db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_hall.do_save_player_hall( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update hall db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_level_recharge.do_save_player_level_recharge( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update level_recharge db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_like.do_save_player_like( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update like db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_carnival.do_save_player_carnival( _player ) < 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db)update carnival db error! player_id: %d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_one_gold_lucky_draw.do_save_player_one_gold_lucky_draw( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update one_gold_lucky_draw db error! player_id:d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_egg.do_save_player_egg( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update egg db error! player_id:d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_festival.do_save_player_festival( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update festival db error! player_id:d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_player_chat_gift.do_save_player_player_chat_gift( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update player_chat_gift db error! player_id:d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_christmas_sell.do_save_player_christmas_sell( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update christmas_sell db error! player_id:d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_christmas_recharge_lucky_draw.do_save_player_christmas_recharge_lucky_draw( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update christmas_recharge_lucky_draw db error! player_id:d", _player.player_id_ )
        database:c_rollback()
        return
    end

    if db_wing_system.do_save_player_wing_system( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update wing_system db error! player_id:d", _player.player_id_ )
        database:c_rollback()
        return
    end
    ...
    if db_activity.do_save_player_activity( _player ) < 0 then 
        c_err( "[db_login](db_migrate -> save_player_to_db)update activity db error! player_id:%d", _player.player_id_ )
        database:c_rollback()
        return
    end
...
--$SAVE_DB_DATA
    ...
    if database:c_commit() ~= 0 then
        c_err( "[db_login](db_migrate -> save_player_to_db) commit transaction failed, player_id: %d", _player.player_id_ )
        return
    end
    time = c_cpu_ms() - time
...
function do_backup_player( _player_id )
   ...
   -- end
    ...
    player.is_db_new_ = false
    player.is_account_new_ =  false
    player.is_player_new_ = false
...
--$GET_DB_DATA
    ...
    local player_tbl_str = utils.serialize_table(player)

    local file_name = string.format("%d.lua", player.player_id_)
    local f = io.open(file_name, "w+") 
    f:write( player_tbl_str )
    f:close()
...
function do_restore_player( _player_id   )

    if not g_db_gmserver_init then 
        g_db_gmserver_init = true 

        local host, user, passwd, port, dbname = c_read_mysql( "sgame_gmserver" )
        if( g_db_gmserver:c_connect( host, user, passwd, port ) < 0 ) then
            c_err( "[dbserver] (do_restore_player) connect database test failed.".. "[" .. host .. ":" .. port .."]".. user )
            return false
        end
        if g_db_gmserver:c_select_db( dbname ) ~= 0 then
            c_err( "[dbserver] (do_restore_player) select database test failed" )
            return false
        end

    end 

    local file_name = string.format("%d.lua", _player_id)
    --local file = io.open(file_name , "r")

    local player_str = ""
    for line in io.lines(file_name) do 
        player_str = player_str .. line .. "\n"
    end 

    local func = loadstring(player_str)
    if not func then 
        return nil 
    end 
    local player = func()
    ...
    return  save_player_to_db( player  )
end
