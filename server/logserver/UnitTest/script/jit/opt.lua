...
local opt_const = require( "jit.opt_const" )
local const_state = nil
local get_const_keys = opt_const.get_const_keys
local begin_const_state = opt_const.begin_const_state
get_const_cfg = opt_const.get_const_cfg
...
local function trylookup(st, t, k)
  ...
  if type(t) == "userdata" then
      t = debug.getmetatable(t) or {}
      t = rawget(t, "__index") or {}
  end
  if type(t) == "table" then
      local v = rawget(t, k)
      if v ~= nil then return v end 
  end 
  -- Ten __index level is enough for our purposes.
  for i = 1, 10, 1 do
...
local handler = {
  ...
  GETUPVAL = function(st, slot, pc, a, b, c)
    ...
    local const_id = get_const_keys( slot[a], optlevel ) 
    if const_id then
        const_state[const_id][a] = pc 
    end
  end,
  ...
  GETTABLE = function(st, slot, pc, a, b, c)
    ...
    local const_id, const_keys = get_const_keys( t, optlevel ) 
    if const_id and const_keys and c < 0 then   -- key must be const(c<0)    
      local parent_pc = const_state[const_id][b] 
      if parent_pc and const_keys[slot[c]] then
        if a == b and pc == parent_pc + 1 then
          st[parent_pc+hints.COMBINE] = false 
        end
        const_keys[slot[c]] = slot[a]
        st[pc+hints.TYPEKEYCONST] = const_keys 
      end
    end
  end,
  ...
  BAND = arithop, BOR = arithop, BXOR = arithop, BSHL = arithop, BSHR = arithop, BNOT = arithop,
...
local function h_opt(st)
  ...
  if not ok then
    ...
    c_err( string.format( "ERROR: jit.opt disabled: %s", err ) )
