...
const char LogClient::className[] = "LogClient";
const bool LogClient::gc_delete_ = false;

Lunar<LogClient>::RegType LogClient::methods[] =
{   
    {NULL, NULL}
};
...
void LogClient::init_func_map()
{
    ...
    func_map_[PT_CONNECT] = &LogClient::on_connect; 
    func_map_[PT_DISCONNECT] = &LogClient::on_disconnect; 
}

void LogClient::send_certify()
{
    BEFORESEND( ar, LG_TYPE_CERTIFY );
    ar.write_string( g_logclient->certify_code_ );
    SEND( ar, g_logclient, 0 );
}

void LogClient::on_connect( Ar& _ar, DPID _dpid, const char* _buf, u_long _buf_size )
{
    send_certify();
    lua_msg_handle( g_luasvr->L(), _buf, _buf_size, PT_CONNECT, _dpid );
}

void LogClient::on_disconnect( Ar& _ar, DPID _dpid, const char* _buf, u_long _buf_size )
{
    lua_msg_handle( g_luasvr->L(), _buf, _buf_size, PT_DISCONNECT, _dpid );

    del_sock( _dpid );
    g_log_connect_thread->on_conn_disconnect();
}

void LogClient::msg_handle( const char* _msg, int _size, int _packet_type, DPID _dpid )
{
    void (LogClient::*pfn)( Ar&, DPID, const char*, u_long ) = NULL;
    TICK(A);
    ...
    {
        ERR(2)( "[LogClient](msg_handle)packet: %d out of range", _packet_type );
    }
    TICK(B);
    mark_tick( _packet_type + TICK_TYPE_PACKET, A, B );
}

void LogClient::lua_msg_handle( lua_State* _L, const char* _msg, int _size, int _packet_type, DPID _dpid )
{
    if( lua_gettop( _L ) != 0 )
    {
    ...
    g_luasvr->get_lua_ref( LoginLuaSvr::LOGCLIENT_MESSAGE_HANDLER );
    LAr lar( _msg, _size );
...
bool LogClientModule::app_class_init()
{
    ...
    APP_GET_STRING( "CertifyCode", g_logclient->certify_code_ );
    APP_END_TABLE();

    if( strlen(ip) <= 0 || !port ) 
    {
        ERR(2)( "[LogClientModule](app_class_init)read cfg err!" );
        return false;
    }

    g_log_connect_thread->init( g_logclient, ip, port, crypt, poll_wait_time, sleep_time );

    if( g_log_connect_thread->start() < 0 )
    {
        ERR(2)( "start connected thread failed" );  
        return false;
    }

    LOG(2)( "===========LogClient Module Start===========");
...
{
    LOG(2)( "===========LogClient Module Destroy===========");
